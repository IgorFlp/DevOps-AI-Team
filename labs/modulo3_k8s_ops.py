...
task_design = Task(
    description="Desenhe o manifesto K8s para o app 'nexus-api-error' com 2 réplicas na porta 80. Imagem com erro e forçar erro em canary analyzer.,ATENÇÃO estamos em um laboratório, utilize obrigatoriamente a imagem publica 'nginx:latest' para todos os containers. ATENÃO 2: Na API V1 do K8s, se for sugerir tempo de espera no probe, utilize obrigatoriamente utilize obrigatoriamente initialDelaySeconds' e nunca apenas 'initialDelay'",
    expected_output="Arquivo YAML criado no disco com sintaxe Kubernetes V1 estrita.",
    agent=architect
)

task_sync = Task(
    description="Realize a reconciliação (Sync) do manifesto 'nexus-api-error-k8s.yaml' no cluster usando o apply_k8s_manifest.",#usando kubectl
